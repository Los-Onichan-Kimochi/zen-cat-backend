...
// @Param        to        query   string  false  "End date (YYYY-MM-DD)"
// @Param        groupBy   query   string  false  "Group by (day, week, month)"
// @Success      200 {object} map[string]interface{} "Community report data"
// @Failure      400 {object} errors.Error "Bad Request"
// @Failure      401 {object} errors.Error "Missing or malformed JWT"
// @Failure      403 {object} errors.Error "Forbidden - Admin role required"
// @Router       /reports/communities [get]
func (a *Api) GetCommunityReport(c echo.Context) error {
	fromStr := c.QueryParam("from")
	toStr := c.QueryParam("to")
	groupBy := c.QueryParam("groupBy")
	if groupBy == "" {
	...
	report, err := a.BllController.Community.GetCommunityReport(from, to, groupBy)
	if err != nil {
		return errors.HandleError(*err, c)
	}
	return c.JSON(200, report)
